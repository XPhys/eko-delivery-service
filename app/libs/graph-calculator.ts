 ...
 */
export function calcShortestRoute(graph, from: string, to: string) {
 ...
 */
export function calcRouteCost(graph, input: string[]) {
    if (input.length <= 1)
 ...
 * @param to destination node (vertex)
 */
export function calcPossibleRoutes(graphArray: any[], from: string, to: string) {
    let path = [];
 ...
 * @param graph reference graph
 */
export function convertGraphToGraphArray(graph) {
    let graphArray = [];
